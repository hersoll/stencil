...
fn multiplication_and_division(id: String, _lang: &str) -> Result<Problem> {
    ...
    let exp3 = IntRange::without_zero(2, exp1 + exp2 - 1)?.random();
    let total_exp = exp1 + exp2 - exp3;
    let question = format!("$({base}^{exp1} dot {base}^{exp2})/{base}^{exp3}$");
    let answer = if total_exp <= 2 {format!("${}$", base.pow(total_exp))} else {format!("${base}^{total_exp}$")};
    let eval_string = if total_exp <= 2 {format!("= {}", base.pow(total_exp))} else {String::new()};
    let solution =
        format!("$ ({base}^{exp1})^{exp2} = {base}^({exp1} dot {exp2}) = {base}^{total_exp} {eval_string} $");
